...
public class ListOfPendingRequestsForRegistration_Facility extends JFrame {
	 ...
	 * Create the frame.
	 */
	public ListOfPendingRequestsForRegistration_Facility(int row) {
		this.row=row;
		...
		lb = new JLabel("UnPaid Partcipant");
		...
		contentPane.add(lb);
				...
				scrollPane.setBounds(10, 106, 1164, 272);
...
void DisplayTableData() {
	 ...
	 ParticipantsDetails_Table.setModel(model);
  	 ...
  	 ParticipantsDetails_Table.getColumnModel().getColumn(8).setCellRenderer(new ButtonRenderer2_FC());
  	 ParticipantsDetails_Table.getColumnModel().getColumn(8).setCellEditor(new ButtonEditor2_FC(new JTextField(),memberShipIds,row,this));
...
class ButtonRenderer2_FC extends JButton implements  TableCellRenderer
{

//CONSTRUCTOR
public ButtonRenderer2_FC() {
...
//BUTTON EDITOR CLASS
class ButtonEditor2_FC extends DefaultCellEditor
{
...
ListOfPendingRequestsForRegistration_Facility parentWindow;
public ButtonEditor2_FC(JTextField txt, ArrayList<String> memberShipIds, int classIndex, ListOfPendingRequestsForRegistration_Facility parentWindow) {
super(txt);
...
private void setPaymentStatus(Boolean value) {
	 ...
	 int tagetIndex=0;
		...
		StartingWindow.facilityClassDataHolder.get(classIndex).getParticipantDetails().get(tagetIndex).paymentStatus=true;
		float setAmount=StartingWindow.facilityClassDataHolder.get(classIndex).getParticipantDetails().get(tagetIndex).PendingPayment;
		StartingWindow.facilityClassDataHolder.get(classIndex).getParticipantDetails().get(tagetIndex).paidAmount+=setAmount;
		StartingWindow.facilityClassDataHolder.get(classIndex).getParticipantDetails().get(tagetIndex).PendingPayment=0;
		new  ListOfPendingRequestsForRegistration_Facility(classIndex);
